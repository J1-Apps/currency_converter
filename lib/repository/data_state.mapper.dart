...
class DataStateMapper extends ClassMapperBase<DataState> {
  ...
  static DataState<T> _instantiate<T>(DecodingData data) {
    throw MapperException.missingSubclass('DataState', 'state', '${data.value['state']}');
  }
...
abstract class DataStateCopyWith<$R, $In extends DataState<T>, $Out, T> implements ClassCopyWith<$R, $In, $Out> {
  $R call();
...
class DataSuccessMapper extends SubClassMapperBase<DataSuccess> {
  ...
  static const Field<DataSuccess, dynamic> _f$data = Field('data', _$data, arg: _arg$data);
...
mixin DataSuccessMappable<T> {
  String toJson() {
    return DataSuccessMapper.ensureInitialized().encodeJson<DataSuccess<T>>(this as DataSuccess<T>);
  }

  Map<String, dynamic> toMap() {
    return DataSuccessMapper.ensureInitialized().encodeMap<DataSuccess<T>>(this as DataSuccess<T>);
  }

  DataSuccessCopyWith<DataSuccess<T>, DataSuccess<T>, DataSuccess<T>, T> get copyWith =>
      _DataSuccessCopyWithImpl(this as DataSuccess<T>, $identity, $identity);
  @override
  String toString() {
    return DataSuccessMapper.ensureInitialized().stringifyValue(this as DataSuccess<T>);
  }

  @override
  bool operator ==(Object other) {
    return DataSuccessMapper.ensureInitialized().equalsValue(this as DataSuccess<T>, other);
  }

  @override
  int get hashCode {
    return DataSuccessMapper.ensureInitialized().hashValue(this as DataSuccess<T>);
  }
}

extension DataSuccessValueCopy<$R, $Out, T> on ObjectCopyWith<$R, DataSuccess<T>, $Out> {
...
abstract class DataSuccessCopyWith<$R, $In extends DataSuccess<T>, $Out, T>
  ...
  @override
  $R call({T? data});
  DataSuccessCopyWith<$R2, $In, $Out2, T> $chain<$R2, $Out2>(Then<$Out2, $R2> t);
}

class _DataSuccessCopyWithImpl<$R, $Out, T> extends ClassCopyWithBase<$R, DataSuccess<T>, $Out>
    implements DataSuccessCopyWith<$R, DataSuccess<T>, $Out, T> {
  _DataSuccessCopyWithImpl(super.value, super.then, super.then2);

  @override
  late final ClassMapperBase<DataSuccess> $mapper = DataSuccessMapper.ensureInitialized();
  @override
  $R call({T? data}) => $apply(FieldCopyWithData({if (data != null) #data: data}));
  @override
  DataSuccess<T> $make(CopyWithData data) => DataSuccess(data.get(#data, or: $value.data));

  @override
  DataSuccessCopyWith<$R2, DataSuccess<T>, $Out2, T> $chain<$R2, $Out2>(Then<$Out2, $R2> t) =>
...
mixin DataEmptyMappable<T> {
  String toJson() {
    return DataEmptyMapper.ensureInitialized().encodeJson<DataEmpty<T>>(this as DataEmpty<T>);
  }

  Map<String, dynamic> toMap() {
    return DataEmptyMapper.ensureInitialized().encodeMap<DataEmpty<T>>(this as DataEmpty<T>);
  }
  ...
  @override
  String toString() {
    return DataEmptyMapper.ensureInitialized().stringifyValue(this as DataEmpty<T>);
  }

  @override
  bool operator ==(Object other) {
    return DataEmptyMapper.ensureInitialized().equalsValue(this as DataEmpty<T>, other);
  }
...
extension DataEmptyValueCopy<$R, $Out, T> on ObjectCopyWith<$R, DataEmpty<T>, $Out> {
  DataEmptyCopyWith<$R, DataEmpty<T>, $Out, T> get $asDataEmpty =>
      $base.as((v, t, t2) => _DataEmptyCopyWithImpl(v, t, t2));
}

abstract class DataEmptyCopyWith<$R, $In extends DataEmpty<T>, $Out, T> implements DataStateCopyWith<$R, $In, $Out, T> {
  @override
  $R call();
  DataEmptyCopyWith<$R2, $In, $Out2, T> $chain<$R2, $Out2>(Then<$Out2, $R2> t);
}

class _DataEmptyCopyWithImpl<$R, $Out, T> extends ClassCopyWithBase<$R, DataEmpty<T>, $Out>
    implements DataEmptyCopyWith<$R, DataEmpty<T>, $Out, T> {
  _DataEmptyCopyWithImpl(super.value, super.then, super.then2);

  @override
  late final ClassMapperBase<DataEmpty> $mapper = DataEmptyMapper.ensureInitialized();
  @override
  ...
  @override
  DataEmptyCopyWith<$R2, DataEmpty<T>, $Out2, T> $chain<$R2, $Out2>(Then<$Out2, $R2> t) =>
